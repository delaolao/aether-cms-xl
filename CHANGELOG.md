...
### 🎨 新前端主题 Jade（青玉）：杂志式布局 + 通栏首屏 + 侧边栏

需求：**保留现有主题的全部页面元素**，但换配色、换元素位置与布局。

**配色**：青玉主色 `#0f766e` / `#14b8a6` + 沙金点缀 `#d9a441` + 暖白底 `#f6f9f8`，正文墨绿灰 `#14342e`。

**布局变化**（相对 ember）：

| 区域 | ember | jade |
|---|---|---|
| 顶栏 | 橙红渐变色块 | 白底**粘性**导航（品牌左 / 菜单中 / 搜索胶囊右），滚动加阴影 |
| 首页 | 有边距的圆角横幅 → 单列卡片网格 | **通栏首屏**轮播 → 分类磁贴（第一块占两格）→ **双栏**（主列头图大卡片 + 卡片网格 / 300px 粘性侧边栏） |
| 侧边栏 | 无 | 分类导航（含篇数）· 热门标签 · 按学段浏览 · 阅读排行 · 站内入口 |
| 列表页 | 纯文字标题 | 渐变页头（分类/标签名 + 说明 + 本页访问数），卡片网格更宽 |
| 文章页 | 单列正文卡片 | 渐变页头 + 正文 + 240px 阅读辅助栏（JS 生成的目录 + 当前小节高亮 + 回到顶部） |
| 页脚 | 一行版权 | 品牌简介 + 站内入口栏 + 版权行 |

**新增 core 支持**：`core/routes/home.js` 增加 `buildSidebarData()`，为首页提供
`sidebar.categories / tags / stages / ranking`（阅读排行在没有任何统计时自动退回「最新发布」，不会出现一排 0）。
其它主题不受影响：模板用「sidebar 是否存在」兜底，缺数据就不渲染。

**元素完整性**：ember 的每个元素都保留 —— 视频封面/时长、媒体徽章、日期·作者·学段·阅读数、副标题或摘要、
阅读全文、标签筛选工作台锚点（`class="post-grid"`）、分页、上一篇/下一篇、相关文章、知识关联（反向链接 +
相关笔记 + 5 个站内入口）、分享条、视频门面、后台管理条 `{{aetherBar}}`、`headerCode`/`footerCode` 注入。
...
> 部署：主题是整目录新增，20 个文件已列入同步清单第十五批；同时改了 `core/routes/home.js`（需**重启**）。

## [0.17.4] - 2026-09-19

### 🧹 首页装修：分类卡片能删了 + 空孤儿卡片自动清理（使用方反馈）

**现象**：删掉分类为「技术」的测试文章后，后台「首页装修」里那张「技术」卡片仍然在。

**原因**（三条叠加，都是设计使然，但组合起来就成了"删不掉"）：

- 分类不是独立实体，是文章 `category` 字段聚合出来的（`collectCategoryCounts()`）；
- 装修页保存时会把**页面上所有分类**（含还没配图的）写进 `content/data/homepage.json`；
- 读取时 `mergeCategoryCards()` 按约定保留「后台配了、但文章里已经没有」的分类（为支持"筹建中"），
  而分类卡片行原先只有「清除配置」——它只清空字段，保存时又被写回。

**改动**：

- 分类卡片行新增**「删除」**按钮（与广告位一致），确认框区分两种语义：
  文章里已无该分类 → 保存后彻底消失；该分类还有文章 → 会以「未配置」形态回到列表（并提示还有几篇）
- 保存时自动丢弃**空孤儿卡片**（文章里已无此分类 + 未上线 + 无图 + 无说明）；
  配过图/说明或已勾选上线的孤儿卡片**仍然保留**，不会被误删
- `homepage-store.js` 读取时**剥掉 UTF-8 BOM**：手工用记事本 / PowerShell `Set-Content -Encoding UTF8`
  改过该文件就会带 BOM，而 `JSON.parse` 遇到 BOM 直接抛错 → 整份配置被**静默当成空配置**
  （本机造测试数据时实测踩到：装修页上广告位与卡片全没了）

**验证**（本地预览 8096，走 HTTP）：造一份含「心理微课（4 篇）」「技术（空孤儿）」「筹建中分类（已配图孤儿）」
的配置 → GET 三条都在（两条 `missing`）→ 模拟点「保存」→ 返回与磁盘文件里「技术」**已被清理**、
「筹建中分类」保留；再把文件写成带 BOM 的 UTF-8 → GET 仍能正确读出配置（加固前会退化成空配置）。

> 需要部署并重启（改到了 `core/api/homepage-api.js` 与 `core/lib/homepage-store.js`）。
> 线上那条历史遗留条目：部署后在装修页点一次「保存」即可自动清掉，也可以按
> `docs/XL-PROJECT.md` 里的一行命令直接改文件（无需重启）。
...
- vendor 了 `tiny-pinyin`（MIT，约 12 KB）到 `core/lib/vendor/tiny-pinyin/`，**不加 npm 依赖** ——
  本项目部署方式是文件同步、不跑 `npm install`，vendor 进仓库后部署只需同步文件
- `core/lib/pinyin.js`：`slugFromTitle()` 两种风格（默认 `word` = 逐字连写、只在词/标点边界加 `-`；
  `syllable` = 每字之间都加 `-`）。实测与线上现有别名**完全一致**：
  `提升毅力 拒绝拖延` → `tishengyili-jujuetuoyan`、`告别后悔，适应开学` → `gaobiehouhui-shiyingkaixue`
- `GET /api/slug?text=…[&mode=]`（需登录）；纯英文标题本地生成，不打扰服务端
- `slug-manager.js`：标题输入后防抖 350ms **自动填充**别名；**只在「别名为空或由系统生成」时填充** ——
  手改过的别名、已存在文章的别名永不被覆盖；另加「拼音生成」按钮（有二次确认，因为会改变链接）
- 服务端兜底：`content-item-manager` 在未提供别名时优先生成拼音

**② 学段：可空 + 下拉选择 + 添加按钮，并修掉「保存草稿后丢失」**

- **数据丢失根因**：编辑器的**加载路径漏了 `stage`**（之前只接了保存路径）。症状链：填好学段 →
  保存草稿触发重新加载 → `stage` 没被读回、输入框被清空 → 再发布时服务端收到空值 →
  `update` 把 frontmatter 里的 `stage` 删掉，表现为「学段丢了」。
  修复：`content-service.js` 的 `loadContent()` 补上 `stage`；并给「加载数据」加保护（用户动过的学段不被回填覆盖）
- 界面：新增已有学段下拉（首项「（未设置）」）+ 输入框 + 「Add」按钮，一致地支持 留空 / 选已有 / 新增

**③ 分类：已有分类下拉**

- 新增 `GET /api/categories`（公开：分类 + 篇数；此前不存在）
- 编辑器侧边栏加「选择已有分类…」下拉（带篇数），避免手打产生「心理微课 / 心理 微课」这类近重复分类

本地验证（预览 8096，走 HTTP）：`/api/slug` 4/4 正确、未登录 401；学段 建→读回「高中」→
再发布仍为「高中」→ 清空后为空；`/api/categories` 返回分类与篇数；编辑器页面含全部新控件。
> 下拉的**交互**（点选 / 添加）需人工在浏览器里点一遍 —— 无浏览器环境无法自动化。

### 🔧 修掉两处「暂存复制」造成的回归

本次用「引擎仓暂存 → 复制到 xl 仓」的工作流，但 `core/api/content-api.js` 与
`core/admin/static/js/i18n.js` 在两仓**已经分叉**，复制把 xl 仓独有的内容覆盖掉了：

- `content-api.js` 丢了 `GET /api/categories` → 首页分类卡与后台分类下拉的数据源变 404
- `i18n.js` 丢了首页装修页的 `nav_homepage` / `hp_save` / `hp_addBanner`

两处均已恢复，并复核 xl 仓工作区只剩本次有意改动的文件。

## [0.17.1] - 2026-09-15

### 🔧 按使用方要求移除分享条里的「打印/PDF」

需求方不需要这个分享选项。

- `core/lib/media/share-bar.js`：删掉 `data-share="print"` 按钮
- `assets/share-bar.js`：删掉对应的 `window.print()` 处理分支
- 分享条现在只剩 5 个目标：复制链接 / 微信（二维码）/ 企业微信（二维码）/ QQ空间 / QQ
- `assets/aether-extras.css` 里的 `@media print` 规则**保留**：访客仍可用 Ctrl+P 打印，
  且打印时依旧自动隐藏分享条与页面装饰，版面干净

本地验证（直接调用 `buildShareBar()`）：生成 5 个分享目标，无 `data-share="print"`、
无「打印」/「PDF」字样，微信二维码与复制链接正常；线上部署前仍是旧版（有该按钮）。

> 注：本次改动只作用于本仓（xl）。引擎仓（xq / root）仍保留该按钮，需要的话可同样移除。
